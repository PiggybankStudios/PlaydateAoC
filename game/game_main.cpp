...
AppState_t InitGame()
{
	...
	#define REGISTER_AOC_DAY(year, day, stateStructType, testPath, realPath, firstFunc, firstTestStr, secondFunc, secondTestStr) numRegisteredDays++;
	#include "day_registration.h"
	
	CreateVarArray(&gl->dayInfos, fixedHeap, sizeof(DayInfo_t), numRegisteredDays);
	
	#define REGISTER_AOC_DAY(yearValue, dayValue, stateStructType, testPath, realPath, firstFunc, firstTestStr, secondFunc, secondTestStr) do \
	{                                                                                                                                         \
		...
		newDayInfo->firstCalculateFunc = (firstFunc);                                                                                         \
		newDayInfo->secondCalculateFunc = (secondFunc);                                                                                       \
		newDayInfo->testInputPath = NewStr(testPath);                                                                                         \
		newDayInfo->realInputPath = NewStr(realPath);                                                                                         \
		newDayInfo->firstTestExpectedResult = NewStr(firstTestStr);                                                                           \
		newDayInfo->secondTestExpectedResult = NewStr(secondTestStr);                                                                         \
	} while(0)
	#include "day_registration.h"
